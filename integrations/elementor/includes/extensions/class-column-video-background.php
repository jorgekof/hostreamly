<?php
/**
 * Hostreamly Column Video Background Extension
 *
 ...
 * Hostreamly Column Video Background Extension
 */
class Hostreamly_Elementor_Column_Video_Background {

    /**
     * Constructor
     */
    public function __construct() {
        add_action('elementor/element/column/section_style/after_section_end', [$this, 'add_video_background_controls']);
        add_action('elementor/frontend/column/before_render', [$this, 'before_render']);
    }

    /**
     * Add video background controls to column
     */
    public function add_video_background_controls(Element_Base $element) {
        ...
        $element->add_control(
            'hostreamly_video_overlay',
            [
                'label' => __('Video Overlay', 'hostreamly-elementor'),
                'type' => Controls_Manager::COLOR,
                'condition' => [
                    'hostreamly_enable_video_bg' => 'yes',
                ],
                'selectors' => [
                    '{{WRAPPER}} .hostreamly-column-video-overlay' => 'background-color: {{VALUE}};',
                ],
            ]
        );
    ...
    public function before_render(Element_Base $element) {
        ...
        if ('yes' === $settings['hostreamly_enable_video_bg'] && !empty($settings['hostreamly_video_url']['url'])) {
            $element->add_render_attribute('_wrapper', 'class', 'hostreamly-video-background-column');
...
new Hostreamly_Elementor_Column_Video_Background();